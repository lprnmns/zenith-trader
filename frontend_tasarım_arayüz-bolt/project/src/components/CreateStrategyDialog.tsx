...
export function CreateStrategyDialog({ open, onOpenChange }: CreateStrategyDialogProps) {
  ...
  const { addStrategy, fetchStrategies } = useStrategiesStore();
  ...
  const onSubmit = async (data: StrategyForm) => {
    ...
    await addStrategy({
      ...
      okxApiSecret: data.apiSecret,
      okxPassphrase: 'UI_FORM',
      positionSize: data.amountPerTrade ?? 1,
      leverage: data.leverage ?? 1,
      allowedTokens: data.allowedTokens ? data.allowedTokens.split(',').map(t => t.trim()) : [],
    });
            ...
            <Tabs value={currentTab} onValueChange={setCurrentTab}>
              ...
              <TabsContent value="trading" className="space-y-6">
                <div className="space-y-4">
                  ...
                  <RadioGroup onValueChange={(value) => setValue('copyMode', value as any)}>
                    <div className="flex items-center space-x-2 p-4 bg-slate-800/50 rounded-lg">
                      <RadioGroupItem value="Perpetual" id="perpetual" />
                      <Label htmlFor="perpetual" className="text-white flex-1">
                        Perpetual Futures
                        <p className="text-sm text-slate-400">Copy leveraged positions with higher risk/reward</p>
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2 p-4 bg-slate-800/50 rounded-lg">
                      <RadioGroupItem value="Spot" id="spot" />
                      <Label htmlFor="spot" className="text-white flex-1">
                        Spot Trading
                        <p className="text-sm text-slate-400">Copy spot trades with direct asset ownership</p>
                      </Label>
                    </div>
                  </RadioGroup>
                </div>
